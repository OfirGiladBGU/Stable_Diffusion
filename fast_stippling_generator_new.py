...
class ImageBasedSampler:
    def __init__(self, images_path, dataset_paths=None, white_threshold: float = 0.95,
        ...
        # Optional Gaussian blur to pre-process images before sampling
        self.gaussian_blur = bool(gaussian_blur)
        self.blur_radius = float(blur_radius)

        # Smart background removal: sample border pixels (top/bottom/left/right), take the
        # majority (mode) grayscale value and treat pixels with that value as background.
    ...
    def __call__(self, res=512, n_points=3000, density_fn=None, oversample=2.0, plot=True, fig_name=None, idx=-1):
        ...
        # Load and resize image (grayscale). We'll compute smart-background from the
        # resized grayscale pixels before any blur, then optionally apply gaussian blur
        ...
        # Convert final image to torch float in [0,1]
        img_arr = torch.from_numpy(np.array(img)).float().to(device) / 255.0

        # Start with threshold-based mask (white areas)
        ...
        if self.smart_background and (mode_val is not None):
            ...
            if tol == 0:
                smart_mask_np = (img_np == mode_val)
            else:
                smart_mask_np = np.abs(img_np.astype(int) - int(mode_val)) <= tol
            smart_mask = torch.from_numpy(smart_mask_np).to(device)
            # ensure boolean dtype
            smart_mask = smart_mask.bool()
            mask = mask | smart_mask
